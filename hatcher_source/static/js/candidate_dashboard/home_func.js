export function applyFilters() {
  ...
  const workTypeElements = document.querySelectorAll('#work_type_filter input:checked');
  const workType = Array.from(workTypeElements).map(el => el.value).join(','); // Collect all selected job types
  ...
  const exp = document.getElementById(id_for_current_exp).innerText


  fetch(`/dashboard/job_search_ajax/?job_type=${jobType}&salary=${salary}&posted_in=${postedIn}&work_type=${workType}&experience=${exp}`, {
    headers: { 'X-Requested-With': 'XMLHttpRequest' }
  })
    .then(response => response.json())
    .then(data => {
      total_jobs.innerText = "";
      total_jobs.innerText = data.total_jobs;
      updateJobListings(data.jobs);
      console.log(data.all_filter)
      updateAllFilter(data.all_filter);
    })
    .catch(error => console.error('Failed to fetch filtered jobs', error));

}
function updateJobListings(jobs) {

  const jobListContainer = document.getElementById('job-list-container');
  jobListContainer.innerHTML = jobs;

  // if (jobs.length === 0) {
  //   jobListContainer.innerHTML = `
  //       <div class="card mb-3">
  //       <svg width="120" class="card-img-top" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function clear_all() {
  ...
  document.getElementById('all_salary').click()
...
function clear_filter(filter_id) {
  if (filter_id == 'experience_slider') {
    document.getElementById('experience_slider').value = 6;
    id_for_current_exp = "experience_value_" + 6;
    document.querySelectorAll(".exp_val").forEach((i, index) => {
      console.log(index)
      if (index == 6) {
        i.classList.remove("visually-hidden")
